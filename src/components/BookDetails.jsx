...
import { X, User, ShieldCheck, Trash2, Edit3, Save, Clock, Send } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto flex flex-col relative border-4 border-white">
        ...
        {/* --- 🖼️ HEADER & EDIT BUTTON (With Text) --- */}
        <div className="h-40 bg-indigo-600 relative flex items-center justify-center shrink-0">
          ...
          <button onClick={onClose} className="absolute top-4 right-4 bg-black/20 p-2 rounded-full text-white hover:bg-black/40 z-10 transition-colors"><X size={20} /></button>
        ...
        <div className="p-6 pt-0 flex-1 overflow-y-auto">
          ...
          {/* --- 🏠 CONDITIONAL VIEW (Owner vs Borrower) --- */}
          {isOwner ? (
            ...
            /* --- 📨 BORROWER VIEW (Request Feature Restored) --- */
            <div className="space-y-4 mb-6">
              {!myRequest || myRequest.status === 'rejected' ? (
                <div className="space-y-4 animate-in slide-in-from-bottom-2">
                  {myRequest?.status === 'rejected' && (
                    <p className="text-[10px] text-rose-500 font-bold uppercase text-center bg-rose-50 py-2 rounded-lg border border-rose-100">Try requesting again with a better message!</p>
                  )}
                  <textarea 
                  ...
                  />
                  <button 
                    onClick={() => { onBorrow(book, msg); setMsg(''); }} 
                    disabled={!msg.trim()} 
                    className="w-full bg-slate-900 text-white py-4 rounded-xl font-black uppercase shadow-xl hover:bg-black transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                ...
                <div className="p-5 rounded-2xl border-2 bg-indigo-50 border-indigo-100 text-center">
                  <p className="font-black text-indigo-800 text-sm uppercase tracking-widest">Status: {myRequest.status.replace('_', ' ')}</p>
