/**
 * 📚 BookShare Pro - Final Production Build (v2.6)
 * * DATA PERSISTENCE FIX:
 * - Fixed: Permanent App ID to prevent data loss on code updates.
 * - Optimized: Local search and class-shortlisting.
 * - Security: MPIN Auth + Admin God-Mode.
 * - Features: Owner Reply, Notifications, Class Filters.
 */
...
/** * ⚠️ IMPORTANT: NEVER CHANGE THIS appId. 
 * Changing this string will make you lose access to all previous data.
 */
...
export default function App() {
  ...
  // 2. Persistent Real-time Listeners
  useEffect(() => {
    if (!user) return;
    
    // Success functions to ensure app doesn't crash on empty data
  ...
  // 3. Memoized Filter Logic (Super Fast)
  ...
  const handleRegister = async (e) => {
    ...
    } catch (err) { setAuthError('Registration Failure'); } finally { setIsLoading(false); }
  };
  ...
  const handleAdminLogin = (e) => {
    ...
    if (adminId.toLowerCase() === 'admin' && adminKey === 'admin9893@') {
      ...
      setIsAuthModalOpen(false);
    } else setAuthError('Invalid Access');
  };
  ...
  const handleRequestBorrow = async (e) => {
    ...
    } catch (e) { alert("System Fail"); } finally { setIsPublishing(false); }
  };
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `School_Library_${new Date().toLocaleDateString()}.csv`);
    link.click();
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-24 overflow-x-hidden selection:bg-blue-100">
            ...
            {authView === 'register' && (
              <form onSubmit={handleRegister} className="space-y-4 text-left">
                ...
                <div className="bg-blue-50 p-5 rounded-3xl border border-blue-100 flex items-center justify-between shadow-sm">
                   <div className="flex items-center gap-2 text-left text-left"><ShieldCheck className="text-blue-600" size={18} /><span className="text-[10px] font-black uppercase text-slate-500 tracking-widest text-left">Hide Number?</span></div>
      ...
      {/* --- DASHBOARD --- */}
      {!appMode ? (
        <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-700 text-center text-center">
          <div className="mb-12 flex flex-col items-center relative text-center text-center">
            <div className="relative mx-auto w-24 h-24 mb-6 text-center text-center">
              <div className="bg-blue-600 w-24 h-24 rounded-[2.5rem] flex items-center justify-center shadow-2xl shadow-blue-200 animate-in slide-in-from-top duration-700 text-center text-center text-center"><BookOpen className="text-white w-12 h-12" /></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-2xl text-center text-center">
            ...
            <button onClick={() => {setAppMode('donation'); setCurrentTab('explore');}} className="group bg-white p-8 rounded-[3rem] shadow-xl border flex flex-col items-center transition-all hover:border-rose-200 hover:-translate-y-2"><Heart size={40} className="mb-4 text-rose-600" /><h2 className="text-2xl font-black text-center text-center text-center">Donation Hub</h2></button>
          </div>
          <div className="mt-12 flex flex-wrap justify-center gap-4 text-center text-center">
            <button onClick={() => setShowCommunityBoard(true)} className="flex items-center gap-3 bg-blue-50/50 px-6 py-3 rounded-full hover:bg-blue-100 transition-all border border-blue-100 group shadow-sm text-center text-center text-center text-center"><Megaphone size={18} className="text-blue-600 group-hover:rotate-12 transition-transform"/><span className="text-[10px] font-black uppercase tracking-widest text-blue-600">Community Board</span></button>
            <button onClick={() => setIsReportingIssue(true)} className="flex items-center gap-3 bg-white px-6 py-3 rounded-full border-2 hover:bg-slate-50 transition-all active:scale-95 group shadow-sm text-center text-center text-center text-center"><Headset size={18} className="text-slate-500 group-hover:text-blue-600 transition-colors"/><span className="text-[10px] font-black uppercase tracking-widest text-slate-500 group-hover:text-slate-800">Admin Support</span></button>
          </div>
        </div>
      ) : (
        <div className="animate-in fade-in duration-500 text-center">
          <header className={`bg-white sticky top-0 z-40 p-4 border-b flex justify-between items-center ${isAdminAuth ? 'border-red-100 border-b-2' : ''}`}>
            <div className="flex items-center gap-4 text-left text-left text-left"><button onClick={() => setAppMode(null)} className="p-2 hover:bg-slate-100 rounded-xl transition-colors text-left text-left text-left"><ChevronLeft size={24}/></button><div><h1 className="text-lg font-black uppercase tracking-tight text-left text-left text-left">{appMode} Hub</h1><div className="text-[9px] font-black uppercase text-slate-400 text-left text-left text-left">{currentTab === 'explore' ? 'Student Library' : 'My Activity'}</div></div></div>
            <div className="flex items-center gap-2 text-left text-left text-left">
              ...
              <button onClick={() => setIsAddingBook(true)} className={`${appMode === 'sharing' ? 'bg-blue-600' : 'bg-rose-600'} text-white h-12 px-5 rounded-2xl flex items-center gap-2 shadow-xl active:scale-90 transition-all text-center text-center text-center`}><Plus size={20}/><span className="text-[10px] font-black uppercase tracking-widest hidden sm:inline">Add Book</span></button>
            </div>
          </header>
          
          {currentTab === 'explore' && (
            <div className="max-w-5xl mx-auto p-4 space-y-4 animate-in slide-in-from-top duration-300 text-center text-center">
              <div className="relative text-center text-center"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/><input placeholder="Search library..." className="w-full pl-12 pr-4 py-4 bg-slate-50 rounded-2xl font-bold focus:bg-white focus:ring-2 focus:ring-blue-100 transition-all outline-none text-left text-left" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
              
              <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 text-center text-center text-center">
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 border-t pt-4 text-center text-center">
                <div className="flex items-center gap-2 px-3 text-slate-400 text-center"><Filter size={14}/> <span className="text-[9px] font-black uppercase tracking-tighter">Class:</span></div>
          ...
          <main className="max-w-5xl mx-auto p-4 pb-24 text-center text-center">
            {currentTab === 'inbox' && isAdminAuth ? (
              <div className="space-y-6 animate-in slide-in-from-bottom duration-500 text-center text-center text-center">
                <div className="flex flex-col gap-4 text-left text-left text-left">
                  <div className="flex justify-between items-center bg-red-50 p-6 rounded-[2.5rem] border border-red-100 shadow-sm text-left text-left"><div><h2 className="text-xl font-black text-red-900 text-left text-left text-left">Admin Control</h2><p className="text-[9px] font-bold text-red-600 uppercase tracking-widest text-left text-left text-left">Broadcast & Records</p></div><button onClick={handleExportCSV} className="bg-red-600 text-white p-4 rounded-2xl shadow-xl active:scale-95 transition-all text-center text-center text-center"><Download size={20}/></button></div>
                  <button onClick={() => setShowAdminBroadcast(true)} className="w-full py-5 bg-slate-900 text-white rounded-[2rem] font-black uppercase text-xs flex items-center justify-center gap-3 shadow-xl active:scale-95 text-center text-center text-center text-center text-center"><Volume2 size={20}/> Broadcast Announcement</button>
                </div>
                {supportMessages.map(msg => <div key={msg.id} className="p-6 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm text-left text-left text-left text-left text-left text-left text-left"><div className="text-sm font-black text-left text-left">@{msg.senderName} (Class {msg.senderClass}) | Mob: {msg.senderMobile}</div><p className="mt-4 text-slate-700 bg-slate-50 p-5 rounded-[2rem] italic text-left text-left text-left">"{msg.message}"</p></div>)}
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 animate-in fade-in duration-500 text-center text-center text-center">
                {filteredBooksList.map(book => (
                  <div key={book.id} onClick={() => setSelectedBook(book)} className="bg-white rounded-[2.5rem] overflow-hidden border shadow-sm cursor-pointer relative group transition-all hover:shadow-xl hover:-translate-y-1 text-center text-center text-center">
                    <div className="aspect-[4/5] bg-slate-100 overflow-hidden relative text-center text-center text-center">
                      {book.imageUrl ? <img src={book.imageUrl} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" loading="lazy" /> : <div className="w-full h-full flex items-center justify-center text-slate-300 font-black text-[10px] text-center text-center">NO COVER</div>}
                      <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-md px-2 py-1 rounded-lg text-[8px] font-black shadow-sm text-slate-900 uppercase text-center text-center text-center">CL: {book.bookClass}</div>
                      {book.waitlist?.length > 0 && <div className="absolute bottom-3 right-3 bg-blue-600 text-white px-2 py-1 rounded-lg text-[8px] font-black shadow-lg text-center text-center text-center">WAIT: {book.waitlist.length}</div>}
                    </div>
                    <div className="p-4 text-left text-left text-left text-left text-left text-left text-left text-left"><h3 className="font-black text-sm truncate text-left text-left text-left">{book.title}</h3><div className="text-[9px] text-slate-400 font-bold mt-2 uppercase text-left text-left text-left text-left">{book.ownerClass} Holder • {calculateDaysDiff(book.since)}d held</div><div className="w-full h-1 bg-slate-100 mt-3 rounded-full overflow-hidden text-left text-left text-left text-left"><div className="bg-emerald-500 h-full transition-all duration-1000 text-left text-left text-left" style={{ width: `${Math.min((book.readingProgress?.pagesRead || 0) / 2, 100)}%` }}></div></div></div>
                  </div>
                ))}
              </div>
            )}
          ...
          <nav className="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-xl border-t p-4 flex justify-around shadow-2xl z-40 text-center text-center text-center text-center">
            ...
            {isAdminAuth ? <button onClick={() => setCurrentTab('inbox')} className={`flex flex-col items-center gap-1 active:scale-90 transition-all ${currentTab === 'inbox' ? 'text-red-600 scale-110' : 'text-slate-300'}`}><Inbox/><span className="text-[8px] font-black uppercase tracking-widest text-center text-center text-center">Admin</span></button> : <button onClick={() => setCurrentTab('activity')} className={`flex flex-col items-center gap-1 active:scale-90 transition-all ${currentTab === 'activity' ? 'text-slate-900 scale-110' : 'text-slate-300'}`}><Bookmark/><span className="text-[8px] font-black uppercase text-center text-center text-center text-center">Activity</span></button>}
          </nav>
        </div>
      )}

      {/* MODAL: BOOK DETAILS (SECURE BORROW + REPLY) */}
      {selectedBook && !showDeleteConfirm && !isUpdatingProgress && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-y-auto">
          <div className={`bg-white w-full max-w-lg rounded-t-[3.5rem] sm:rounded-[3.5rem] overflow-hidden animate-in slide-in-from-bottom duration-500 my-auto shadow-2xl ${isAdminAuth ? 'border-4 border-red-100' : ''} text-left text-left text-left text-left`}>
            <div className="h-64 relative bg-slate-200 text-left text-left text-left text-left">
              {selectedBook.imageUrl ? <img src={selectedBook.imageUrl} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-300 font-black uppercase text-xs text-center text-center text-center">No Cover</div>}
              <div className="absolute top-6 right-6 flex gap-2 text-left text-left text-left text-left">{(isAdminAuth || (selectedBook.ownerId === user.uid && selectedBook.history?.length === 1)) && <button onClick={() => setShowDeleteConfirm(true)} className="bg-red-500 text-white w-10 h-10 rounded-xl flex items-center justify-center shadow-lg active:scale-90 transition-all text-center text-center text-center"><Trash2 size={20} /></button>}<button onClick={() => setSelectedBook(null)} className="bg-black/20 backdrop-blur-md text-white w-10 h-10 rounded-xl flex items-center justify-center shadow-lg text-center text-center text-center text-center text-center text-center"><X size={20} /></button></div>
            </div>
            <div className="p-8 bg-white max-h-[90dvh] overflow-y-auto no-scrollbar text-left text-left text-left text-left text-left text-left">
              <h2 className="text-2xl font-black mb-1 text-left text-left text-left text-left text-left">{selectedBook.title}</h2>
              <div className="text-slate-400 font-bold text-xs mb-8 uppercase tracking-widest text-left flex items-center gap-2 text-left text-left text-left text-left text-left text-left"><div className="w-4 h-0.5 bg-slate-200 text-left text-left text-left text-left"></div> by {selectedBook.author}</div>
              
              <div className="bg-slate-50 p-6 rounded-[3rem] border border-slate-100 flex items-center gap-4 shadow-sm text-left mb-8 relative text-left text-left text-left text-left text-left">
                 <div className={`p-3 rounded-2xl ${isAdminAuth ? 'bg-red-100 text-red-600' : 'bg-white text-blue-600 shadow-sm'} text-left text-left text-left text-left`}><Phone size={20}/></div>
                 <div className="text-left text-sm font-black tracking-widest text-slate-800 text-left text-left text-left text-left">
                    {(isAdminAuth || !selectedBook.isPrivate || selectedBook.ownerId === user.uid) ? (
                      <>{selectedBook.contact}{selectedBook.isPrivate && <span className="bg-red-600 text-white px-2 py-0.5 rounded text-[8px] uppercase font-black ml-2 animate-pulse text-left text-left">God Mode</span>}</>
                    ) : (
                      <span className="italic flex items-center gap-2 text-slate-400 font-bold text-left text-left text-left text-left text-left"><EyeOff size={14}/> Hidden Number</span>
                    )}
                 </div>
              </div>
              ...
              {user.uid !== selectedBook.ownerId && !selectedBook.waitlist?.some(r => r.uid === user.uid) && (
                 <form onSubmit={handleRequestBorrow} className="space-y-4 mb-8 bg-blue-50 p-6 rounded-[2.5rem] border border-blue-100 text-left text-left text-left text-left">
                    <p className="text-[10px] font-black uppercase text-blue-600 tracking-widest mb-2 text-left text-left text-left">Interested? Send Borrow Request</p>
                    <textarea required placeholder="Message for owner..." className="w-full p-4 bg-white border rounded-2xl text-sm outline-none focus:ring-2 ring-blue-200 resize-none text-left text-left text-left text-left text-left" value={borrowMsg} onChange={e => setBorrowMsg(e.target.value)} />
                    <input required type="tel" maxLength={10} placeholder="Verify your mobile number" className="w-full p-4 bg-white border rounded-2xl text-sm outline-none focus:ring-2 ring-blue-200 text-left text-left text-left text-left" value={borrowMobile} onChange={e => handleNumericInput(e.target.value, setBorrowMobile, 10)} />
                    <button type="submit" disabled={isPublishing || borrowMobile.length !== 10} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-2 shadow-lg active:scale-95 text-center text-center text-center">
                       {isPublishing ? <Loader2 className="animate-spin" size={16}/> : <><Send size={16}/> Send Request</>}
                    </button>
                 </form>
              )}

              {/* OWNER'S VIEW - REPLY LOGIC */}
              {(user.uid === selectedBook.ownerId || isAdminAuth) && selectedBook.waitlist?.length > 0 && (
                <div className={`mt-8 space-y-4 border-2 p-6 rounded-[2.5rem] shadow-sm text-left text-left text-left ${isAdminAuth ? 'bg-red-50/20 border-red-100' : 'bg-blue-50/20 border-blue-100'}`}>
                   <p className="text-[10px] font-black uppercase text-slate-400 mb-4 flex items-center gap-2 text-left text-left text-left text-left"><Users size={16}/> Student Requests ({selectedBook.waitlist.length})</p>
                   {selectedBook.waitlist.map((req, i) => (
                     <div key={i} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm mb-4 text-left text-left text-left text-left">
                        <div className="flex justify-between items-start mb-3 text-left text-left">
                           <div className="text-left text-left text-left text-left"><div className="text-sm font-black text-left text-left">@{req.name} (Cl: {req.studentClass})</div><div className="text-[8px] font-bold text-slate-400 uppercase mt-1 text-left text-left">Mobile: {req.contact}</div></div>
                           <button onClick={() => handleApproveFromWaitlist(req)} className="bg-blue-600 text-white px-5 py-2.5 rounded-2xl text-[9px] font-black uppercase shadow-lg transition-all active:scale-95 text-center text-center text-center">Approve</button>
                        </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-50 text-left text-left text-left">
                          {req.ownerReply ? (
                            <div className="bg-emerald-50 p-3 rounded-2xl flex items-center gap-2 text-left text-left text-left">
                               <CheckCircle2 size={14} className="text-emerald-600"/>
                               <p className="text-[10px] font-bold text-emerald-800 text-left text-left">Sent: {req.ownerReply}</p>
                            ...
                            <div className="flex gap-2 text-left text-left text-left">
                               ...
                               <button onClick={() => handleOwnerReply(req.uid)} className="bg-slate-900 text-white p-2.5 rounded-xl active:scale-90 transition-all text-center text-center text-center"><Send size={14}/></button>
                            </div>
                          )}
                        </div>
                     </div>
      ...
      {/* MODAL: ADD BOOK (Integrated Class) */}
      {isAddingBook && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[70] flex items-center justify-center p-4">
          <form onSubmit={handleAddBook} className="bg-white w-full max-w-md rounded-[3.5rem] p-10 shadow-2xl max-h-[90dvh] overflow-y-auto no-scrollbar animate-in duration-300 text-left text-left text-left">
            <div className="flex justify-between items-center mb-8 text-left text-left text-left text-left text-left text-left"><h2 className="text-2xl font-black tracking-tight text-left text-left text-left text-left text-left text-left text-left text-left">List a Book</h2><button type="button" onClick={() => setIsAddingBook(false)} className="p-2 bg-slate-100 rounded-xl hover:bg-slate-200 transition-all text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center"><X size={20}/></button></div>
            <div className="space-y-5 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
              <div onClick={() => !isLoading && fileInputRef.current.click()} className="aspect-video bg-slate-50 border-4 border-dashed border-slate-100 rounded-[2.5rem] flex flex-col items-center justify-center cursor-pointer overflow-hidden group hover:border-blue-200 transition-all relative text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left">
                {(isLoading || isPublishing) && <div className="absolute inset-0 bg-white/80 flex items-center justify-center z-10 font-black uppercase text-[10px] animate-pulse text-center text-center text-center text-center text-center text-center text-center">Processing...</div>}
                {bookImageUrl ? <img src={bookImageUrl} className="w-full h-full object-cover" /> : <><Camera size={48} className="text-slate-200 mb-2 group-hover:text-blue-300 transition-colors text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center" /><div className="text-[10px] font-black uppercase tracking-widest text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center">Add Book Photo</div></>}<input type="file" ref={fileInputRef} className="hidden" accept="image/*" capture="environment" onChange={handleImageUpload} />
              </div>
              <input required placeholder="Book Title" className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold outline-none focus:border-blue-600 transition-all shadow-sm text-left text-left text-left text-left text-left text-left text-left text-left shadow-inner text-left text-left text-left text-left text-left text-left" value={newBookTitle} onChange={e => setNewBookTitle(e.target.value)} />
              
              <div className="grid grid-cols-2 gap-3 text-left text-left text-left text-left text-left text-left text-left text-left">
                 <div><label className="text-[8px] font-black uppercase text-slate-400 ml-2 text-left text-left text-left">Category</label><select className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black uppercase text-[10px] outline-none shadow-sm shadow-inner text-left text-left text-left text-left text-left text-left text-left" value={newBookCategory} onChange={e => setNewBookCategory(e.target.value)}>{CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                 <div><label className="text-[8px] font-black uppercase text-slate-400 ml-2 text-left text-left text-left">Target Class</label><select className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black uppercase text-[10px] outline-none shadow-sm shadow-inner text-left text-left text-left text-left text-left text-left text-left" value={newBookClass} onChange={e => setNewBookClass(e.target.value)}>{CLASSES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
              </div>

              <textarea placeholder="Advice or condition notes..." className="w-full h-32 p-5 bg-slate-50 border-2 border-slate-100 rounded-3xl font-bold text-sm outline-none focus:border-blue-600 resize-none transition-all shadow-sm text-left text-left text-left text-left text-left text-left text-left text-left shadow-inner text-left text-left text-left text-left text-left text-left" value={newBookRemark} onChange={e => setNewBookRemark(e.target.value)} />
              <button type="submit" disabled={isLoading || isPublishing} className={`w-full py-5 rounded-[2rem] font-black uppercase text-xs text-white shadow-2xl active:scale-95 transition-all text-center text-center text-center text-center text-center text-center text-center ${appMode === 'sharing' ? 'bg-blue-600 shadow-blue-100' : 'bg-rose-600 shadow-rose-100'} disabled:opacity-50 text-center text-center text-center text-center text-center text-center text-center text-center text-center`}>{isPublishing ? "Publishing..." : "List in Library"}</button>
            </div>
          </form>
        </div>
      )}
